...
The MCP server must continue to work when the editor window is closed because the app remains resident in the tray. Only explicitly quitting the tray app should shut down the MCP server.
...
- run semantic restyle workflows
- promote selections or targets into styles or variables
...
- v1 accepts that any process or person with local-machine access to the listener can inspect or mutate projects when MCP is enabled
...
- it can mutate the same project through the same command/query core
- design-system workflows behave the same in UI and MCP paths
- it remains available after the editor window closes until the user explicitly quits the tray app
