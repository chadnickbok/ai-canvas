...
function createDocumentWithInspectorFixtures(): RendererDocument {
  ...
  document.nodes.scene_home.render_style.backgroundImage =
  ...
  document.nodes.rect_loose.render_style.backgroundImage =
    'url(asset://asset_loose)';
...
function createDocumentWithSelectionInspectorFixtures(): RendererDocument {
  ...
  document.nodes.text_title.authoring.local_values[
    'node.typography.font_size'
  ] = 32;
  document.nodes.text_title.authoring.variable_bindings['node.text.color'] =
    'color_text';
...
function renderIntoDom(
  ...
  options: {
    ...
    ) => Promise<ReturnType<typeof ok<CommandResult>>>;
...
function assignInteractionGeometry(harness: RenderHarness) {
  ...
  const sceneFrame = harness.container.querySelector(
    '[data-node-id="scene_home"]',
  ) as HTMLElement;
  const heroRectangle = harness.container.querySelector(
    '[data-node-id="rect_hero"]',
  ) as HTMLElement | null;
  const nestedFrame = harness.container.querySelector(
    '[data-node-id="frame_nested"]',
  ) as HTMLElement | null;
  const titleText = harness.container.querySelector(
    '[data-node-id="text_title"]',
  ) as HTMLElement | null;
...
describe('DocumentWorkspaceScreen', () => {
  it('shows the empty-state callout for a blank document', () => {
    ...
    try {
      ...
      const viewportHint = harness.container.querySelector(
      ...
      expect(viewportHint.getAttribute('data-viewport-hint-visible')).toBe(
        'true',
      );
      expect(
        harness.container.querySelector('[data-viewport-frame="true"]'),
      ).not.toBeNull();
    } finally {
      harness.cleanup();
    }
  ...
  it('renders committed scene content through the shared workspace surface', () => {
    ...
    try {
      ...
      const rectangle = harness.container.querySelector(
        '[data-node-id="rect_hero"]',
      ) as HTMLElement;
      ...
      expect(
        countMatches(harness.container.textContent ?? '', /1 scene/g),
      ).toBe(1);
  ...
  it('renders the floating canvas tool bar with selection active by default', () => {
    ...
    try {
      const toolbar = harness.container.querySelector(
  ...
  it('uses the grab tool to pan the viewport with primary drag', () => {
    ...
    try {
      ...
      const grabTool = getCanvasToolButton(
        harness,
        'grab',
  ...
  it('creates a loose top-level rectangle from the toolbar on blank canvas click and returns to selection', async () => {
    ...
    try {
      ...
      const selectionTool = getCanvasToolButton(
        harness,
        'selection',
      ) as HTMLButtonElement;
  ...
  it('creates a child frame when the frame tool clicks an existing frame', async () => {
    ...
    try {
      ...
      const frameTool = getCanvasToolButton(
        harness,
        'frame',
      ) as HTMLButtonElement;
  ...
  it('creates a sibling text node after clicking an existing rectangle', async () => {
    ...
    try {
      ...
      const textTool = getCanvasToolButton(
        harness,
        'text',
      ) as HTMLButtonElement;
  ...
  it('patches nested frame parents to relative positioning before creating absolute children', async () => {
    ...
    const harness = renderIntoDom(
      createActiveProject(createDocumentWithNestedFrame()),
      {
        onApplyCommands,
      },
    );
  ...
  it('renders the layers inspector in canvas order with type-specific icons', () => {
    const harness = renderIntoDom(
      createActiveProject(createDocumentWithInspectorFixtures()),
    );

    try {
      const layerRows = [
        ...harness.container.querySelectorAll('[data-layer-row="true"]'),
      ];
      ...
      expect(
        layerRows.map((row) => row.getAttribute('data-layer-node-id')),
      ).toEqual(['scene_home', 'rect_hero', 'text_title', 'rect_loose']);

      expect(
        sceneRow?.querySelector('[data-layer-icon-type="frame"]'),
      ).not.toBeNull();
      expect(
        sceneRow?.querySelector('[data-layer-icon-direction="row"]'),
      ).not.toBeNull();
      expect(
        heroRow?.querySelector('[data-layer-icon-type="rectangle"]'),
      ).not.toBeNull();
      expect(
        titleRow?.querySelector('[data-layer-icon-type="text"]'),
      ).not.toBeNull();
      expect(
        looseRow?.querySelector('[data-layer-icon-type="image"]'),
      ).not.toBeNull();
  ...
  it('shows layout and auto layout details for scene-root selection', async () => {
    ...
    try {
      ...
      const inspector = getSelectionInspector(harness) as HTMLElement;
      const layoutSection = getInspectorSection(
        harness,
        'layout',
      ) as HTMLElement;
      const autoLayoutSection = getInspectorSection(
        harness,
        'auto-layout',
      ) as HTMLElement;
      ...
      expect(getInspectorMetric(layoutSection, 'x')?.textContent).toContain(
        '80',
      );
      expect(getInspectorMetric(layoutSection, 'y')?.textContent).toContain(
        '80',
      );
      expect(getInspectorMetric(layoutSection, 'w')?.textContent).toContain(
        '390',
      );
      expect(getInspectorMetric(layoutSection, 'h')?.textContent).toContain(
        '844',
      );
  ...
  it('shows flex item and text details for in-flow child selection', async () => {
    ...
    try {
      ...
      const flexItemSection = getInspectorSection(
        harness,
        'flex-item',
      ) as HTMLElement;
  ...
  it('shows x, y, width, and height for absolute selections without debug fields', async () => {
    ...
    try {
      ...
      const appearanceSection = getInspectorSection(
        harness,
        'appearance',
      ) as HTMLElement;
  ...
  it('hides the layers pane and shows a floating reveal control', () => {
    ...
    try {
      ...
      expect(
        harness.container.querySelector('[data-layers-inspector="true"]'),
      ).not.toBeNull();
      ...
      expect(
        harness.container.querySelector('[data-layers-overlay-panel="true"]'),
      ).toBeNull();
      expect(getShowLayersToggle(harness)?.getAttribute('aria-label')).toBe(
        'Show layers panel',
      );
      expect(
        harness.container.querySelector('[data-viewport-frame="true"]'),
  ...
  it('supports horizontal scrolling for deep layer indentation', () => {
    const harness = renderIntoDom(
      createActiveProject(createDocumentWithDeepHierarchy()),
    );
  ...
  it('fits initial content into the viewport and lets the user pan, zoom, and refit', async () => {
    ...
    try {
      ...
      const contentBounds = resolveTopLevelContentBounds(
  ...
  it('keeps major grid lines visible below 25% zoom while hiding minor lines', async () => {
    ...
    try {
      ...
      const majorLines = harness.container.querySelectorAll(
        '[data-grid-line-kind="major"]',
      );
      const minorLines = harness.container.querySelectorAll(
        '[data-grid-line-kind="minor"]',
      );
  ...
  it('fades the viewport hint after the first canvas interaction and restores it for a new document', async () => {
    ...
    try {
      ...
      expect(
        viewportHintBefore.getAttribute('data-viewport-hint-visible'),
      ).toBe('true');
      ...
      expect(viewportHintAfter.getAttribute('data-viewport-hint-visible')).toBe(
        'false',
      );
      ...
      expect(
        viewportHintReopened.getAttribute('data-viewport-hint-visible'),
      ).toBe('true');
  ...
  it('syncs canvas selection into the layers inspector and re-expands collapsed ancestors', () => {
    ...
    try {
      ...
      expect(
        getLayerDisclosure(harness, 'scene_home')?.getAttribute(
          'data-expanded',
        ),
      ).toBe('true');
      expect(
        getLayerRow(harness, 'rect_hero')?.getAttribute('data-layer-selected'),
      ).toBe('true');
  ...
  it('scrolls the hierarchy row into view for offscreen canvas selections', () => {
    ...
    try {
      ...
      const deepCanvasNode = harness.container.querySelector(
        '[data-node-id="text_leaf"]',
      ) as HTMLElement;
      ...
      expect(
        getLayerRow(harness, 'text_leaf')?.getAttribute('data-layer-selected'),
      ).toBe('true');
  ...
  it('does not scroll the hierarchy row when a canvas selection is already visible', () => {
    ...
    try {
      ...
      const heroCanvasNode = harness.container.querySelector(
        '[data-node-id="rect_hero"]',
      ) as HTMLElement;
  ...
  it('does not auto-scroll the hierarchy for hierarchy-driven selection', () => {
    ...
    try {
      ...
      expect(
        getLayerRow(harness, 'scene_home')?.getAttribute('data-layer-selected'),
      ).toBe('true');
      expect(scrollIntoViewMock).not.toHaveBeenCalled();
    } finally {
      harness.cleanup();
  ...
  it('commits southeast resize drags for loose top-level nodes through update_node', async () => {
    ...
    try {
      ...
      expect(
        harness.container.querySelector(
          '[data-interaction-preview="true"][data-node-id="rect_loose"]',
        ),
      ).not.toBeNull();
  ...
  it('keeps in-flow scene children selectable but without drag handles or mutation', () => {
    ...
    try {
      ...
      expect(
        harness.container.querySelector(
          '[data-interaction-outline="selected"][data-node-id="rect_hero"]',
        ),
      ).not.toBeNull();
      expect(
        harness.container.querySelector(
          '[data-interaction-handle][data-node-id="rect_hero"]',
        ),
      ).toBeNull();
