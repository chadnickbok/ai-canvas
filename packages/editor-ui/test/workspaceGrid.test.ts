...
describe("workspace grid helpers", () => {
  ...
  it("applies half-pixel offsets for one-pixel strokes on standard displays", () => {
    expect(snapGridLineToDevicePixel(10.2, { devicePixelRatio: 1, strokeWidth: 1 })).toBe(10.5);
    expect(snapGridLineToDevicePixel(10.2, { devicePixelRatio: 2, strokeWidth: 1 })).toBe(10);
  });
});
