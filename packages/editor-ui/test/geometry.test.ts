...
describe('geometry helpers', () => {
  ...
  it('prefers measured DOM rects, then computed_layout, then authored geometry', () => {
    ...
    expect(
      resolveNodeCanvasRectWithSource(document, 'rect_loose', null, 1),
    ).toEqual({
      rect: {
        bottom: 280,
        height: 120,
    ...
    expect(
      resolveNodeCanvasRectWithSource(document, 'rect_non_pixel', null, 1),
    ).toBeNull();
  });
  ...
  it('allows mutation only for directly manipulable nodes', () => {
    ...
    expect(
      isNodeDirectlyManipulable(document, document.nodes.scene_home, true),
    ).toBe(true);
    expect(
      isNodeDirectlyManipulable(document, document.nodes.rect_loose, true),
    ).toBe(true);
    expect(
      isNodeDirectlyManipulable(document, document.nodes.rect_absolute, true),
    ).toBe(true);
    expect(
      isNodeDirectlyManipulable(document, document.nodes.rect_flow, true),
    ).toBe(false);
    expect(
      isNodeDirectlyManipulable(document, document.nodes.primitive_node, true),
    ).toBe(false);
    expect(
      isNodeDirectlyManipulable(document, document.nodes.rect_non_pixel, true),
    ).toBe(false);
    expect(
      isNodeDirectlyManipulable(
        document,
    ...
    expect(
      isNodeDirectlyManipulable(document, document.nodes.rect_loose, false),
    ).toBe(false);
  });
});
