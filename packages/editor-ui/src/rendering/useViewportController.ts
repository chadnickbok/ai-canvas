...
export function useViewportController({
  ...
  const [viewportSize, setViewportSize] =
    useState<ViewportSize>(EMPTY_VIEWPORT_SIZE);
  ...
  const contentBounds = useMemo(
    () => resolveTopLevelContentBounds(document),
    [document],
  );
  ...
  const isCurrentWorkspaceViewport =
    viewportState.workspaceIdentity === workspaceIdentity;
  const viewport =
    isCurrentWorkspaceViewport && viewportState.viewport
    ...
    (input: {
      ...
      if (input.ctrlKey || input.metaKey) {
        ...
        const zoomFactor = Math.exp(
          -input.deltaY * MODIFIED_WHEEL_ZOOM_SENSITIVITY,
        );
  ...
  const handlePointerDown = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      const shouldStartDrag =
        event.button === 1 ||
        (event.button === 0 && (isSpacePressed || isPanModePinned));
