...
function getSortDate(project: ProjectSummary, sortMode: SortMode): Date | null {
  return parseDate(
    sortMode === 'created' ? project.createdAt : project.updatedAt,
  );
}
...
function buildProjectSections(
  ...
  for (const project of sortedProjects) {
    sections
      .get(bucketProject(getSortDate(project, sortMode), now))
      ?.push(project);
  }
...
function EmptyPanel({
  ...
  return (
    ...
    >
      <strong className="text-[18px] font-semibold tracking-[-0.03em] text-[#111111]">
        {title}
      </strong>
      <p className="m-0 max-w-[32rem] text-[15px] leading-7 text-black/66">
        {body}
      </p>
    </div>
  );
...
function TryCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="flex flex-col gap-2 border border-black/16 bg-white px-4 py-4">
      <span className="ui-mono text-[12px] uppercase tracking-[0.12em] text-black/44">
        {title}
      </span>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        <h3 className="m-0 text-[26px] font-semibold tracking-[-0.04em] text-[#111111]">
          {title}
        </h3>
        <p className="m-0 max-w-[38rem] text-[15px] leading-8 text-[#111111]">
          {body}
        </p>
      ...
      {previewVariant === 'codex' ? (
        <div className="flex h-[210px] flex-col overflow-hidden border border-black/16 bg-white">
          <div className="flex h-[34px] items-center justify-between border-b border-black/10 bg-[var(--chrome-surface-muted)] px-3">
            ...
            <span className="ui-mono text-[11px] text-black/34">
              screenshot slot
            </span>
          </div>
...
export function ProjectLibraryScreen(props: ProjectLibraryScreenProps) {
  ...
  const [isCreateProjectDialogOpen, setCreateProjectDialogOpen] =
    useState(false);
  const [createProjectName, setCreateProjectName] = useState('');
  const [createProjectSubmissionError, setCreateProjectSubmissionError] =
  ...
  const projectSections = isReady
    ...
    : [];
  const hasFilteredResults = projectSections.some(
  ...
  const measurementWarning =
    ...
    !props.runtimeCapabilities.measurementSurfaceAvailable
  ...
  const modeWarning =
    props.runtimeCapabilities?.mode === 'read_only'
      ...
      : null;
  const createProjectValidation = createProjectInputSchema.safeParse({
    name: createProjectName,
  });
  const createProjectValidationMessage = createProjectValidation.success
    ? null
    : (createProjectValidation.error.issues[0]?.message ??
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      nodeMap[guideTarget]?.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    });
  ...
  const handleCreateProjectSubmit = async (
    ...
    try {
      await Promise.resolve(
        props.onCreateProject(createProjectValidation.data),
      );
          ...
          <section className="grid gap-8 border-b border-black/12 pb-6 xl:grid-cols-[minmax(0,1fr)_280px]">
            <div className="flex flex-col gap-4">
              ...
              <p className="m-0 max-w-[760px] text-[20px] leading-[1.55] text-black/72">
                Set up Model Context Protocol in your client, point it at AI
                Canvas Desktop, and connect the same live project session the
                editor is using.
              </p>
              <p className="m-0 max-w-[760px] text-[16px] leading-8 text-[#111111]">
                MCP is the bridge that lets tools like Claude, Codex, and Gemini
                inspect project state and send deterministic mutations through
                the same document core as the UI. For AI Canvas Desktop,
                inspection can continue when the window is closed, but
                write-capable actions require the editor window to be open so
                the measurement surface is available.
              </p>
            </div>
            ...
            <aside className="flex flex-col gap-3 border border-black/16 bg-white px-5 py-5">
              ...
              <p className="m-0 text-[14px] leading-8 text-[#111111]">
                Keep the AI Canvas Desktop window open for edits that depend on
                layout measurement.
              </p>
              <p className="m-0 text-[14px] leading-8 text-[#111111]">
              ...
              <p className="m-0 text-[14px] leading-8 text-[#111111]">
                If your client supports screenshots in docs, include the
                app-specific setup screen here.
              </p>
            </aside>
          </section>
          ...
          <section className="grid gap-8 border-b border-black/12 pb-7 xl:grid-cols-[620px_minmax(0,1fr)]">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 text-[15px] leading-8 text-[#111111]">
                <p className="m-0">
                  <span className="ui-mono mr-4 text-[12px] text-black/34">
                    01
                  </span>
                  Open the MCP or developer settings for your client. Most tools
                  expose a place to register local servers and their startup
                  config.
                </p>
                <p className="m-0">
                  <span className="ui-mono mr-4 text-[12px] text-black/34">
                  ...
                  Copy the exact AI Canvas MCP endpoint from the app. For the
                  local bridge this includes the path, for example
                  <span className="ui-mono mx-2 text-[13px]">
                ...
                <p className="m-0">
                  ...
                  Restart or reload the client if required, then verify that the
                  AI Canvas MCP appears as an available server or tool source.
                </p>
                <p className="m-0">
                  <span className="ui-mono mr-4 text-[12px] text-black/34">
                    04
                  </span>
                  Open a project in AI Canvas Desktop before using write-capable
                  actions so the live document and measurement surface are
                  ready.
                </p>
              </div>
            </div>
            ...
            <aside className="flex flex-col gap-3 border border-black/16 bg-white px-5 py-5">
              ...
              <p className="m-0 text-[15px] leading-8 text-[#111111]">
                Write actions are routed through the canonical command system,
                not a separate automation model.
              </p>
          ...
          <div className="flex flex-col gap-3 border-b border-black/8 py-5">
            <div className="flex items-center gap-2">
              ...
              <UtilityButton
                className={cn(
                  sortMode === 'updated' &&
                    'border-black bg-white text-[#111111]',
                )}
                disabled={!isReady || props.projects.length === 0}
                onClick={() => {
                  setSortMode('updated');
            ...
            <div className="flex max-w-[620px] flex-col gap-6">
              <div className="flex flex-col gap-4 border-b border-black/12 pb-5">
                ...
                <p className="m-0 text-[20px] leading-[1.55] text-black/72">
                  Local-first, scene-first mockup editing with a shared document
                  core for both humans and AI agents.
                </p>
              </div>

              <div className="flex flex-col gap-5 text-[16px] leading-8 text-[#111111]">
                <p className="m-0">
                  AI Canvas Desktop is a desktop workspace for building
                  structured mockups, scenes, and reusable design-system data on
                  your machine. The editor UI and MCP operate on the same
                  document schema, command system, and rendering model, so edits
                  stay deterministic and inspectable.
                </p>
                <p className="m-0">
                  Connect MCP to get the full experience. Claude, Codex, and
                  Gemini can inspect the same live project session you see here
                  and apply changes through the same mutation path as the UI.
                </p>
              </div>
              ...
              {hasMcpError ? (
                <div className="border border-black/18 bg-black/[0.03] px-5 py-4 text-[14px] leading-7 text-[#111111]">
                  MCP startup failed. {props.mcpStatus?.errorMessage} External
                  tools stay unavailable until the conflict is resolved.
                </div>
              ) : null}
            ...
            <aside className="flex max-w-[280px] flex-col gap-4 pt-[108px]">
              <div className="flex flex-col gap-2 border-b border-black/12 pb-4">
                ...
                <p className="m-0 text-[15px] leading-7 text-black/62">
                  Once MCP is connected, use your preferred client to try a few
                  concrete tasks.
                </p>
              </div>
          ...
          >
            <div className="flex flex-col gap-2">
              ...
              <p className="m-0 text-[15px] leading-7 text-black/66">
                Projects are created on your machine and become the active
                session for the editor and MCP.
              </p>
            </div>
              ...
              <div className="min-h-[28px] text-[13px] leading-6 text-[#111111]">
                {createProjectInlineError ? (
                  ...
                  <span className="text-black/52">
                    Required. Up to 120 characters. The name is used for the
                    document and project summary.
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center justify-end gap-3">
                <button
                  className={cn(
                    ...
                    props.isBusy &&
                      'cursor-not-allowed opacity-45 hover:border-black/18',
                  )}
                  disabled={props.isBusy}
