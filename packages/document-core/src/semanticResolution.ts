...
export const RENDER_KEY_TO_NODE_SEMANTIC_SLOT = Object.fromEntries(
  Object.entries(NODE_SEMANTIC_SLOT_TO_RENDER_KEY).map(([slot, renderKey]) => [
    renderKey,
    slot,
  ]),
) as Partial<
  Record<
    (typeof NODE_SEMANTIC_SLOT_TO_RENDER_KEY)[NodeSemanticSlot],
    NodeSemanticSlot
  >
>;
...
const TYPOGRAPHY_SLOT_TO_FIELD = {
  ...
  Record<NodeSemanticSlot, keyof TypographyTokenValue>
>;
...
export type ResolvedSemanticValue<
  TSlot extends RendererSemanticSlot = RendererSemanticSlot,
> = {
  slot: TSlot;
...
export type ResolvedCanvasSemanticValue =
  ResolvedSemanticValue<CanvasSemanticSlot>;
export type ResolvedNodeSemanticValue = ResolvedSemanticValue<NodeSemanticSlot>;
export type ResolvedCanvasSemanticState = Record<
  CanvasSemanticSlot,
  ResolvedCanvasSemanticValue
>;
export type ResolvedNodeSemanticState = Partial<
  Record<NodeSemanticSlot, ResolvedNodeSemanticValue>
>;
...
export function getNodeSemanticSlotForRenderKey(
  renderKey: string,
): NodeSemanticSlot | undefined {
...
function resolveTypographyValue(
  ...
  const field =
    TYPOGRAPHY_SLOT_TO_FIELD[
      requestedSlot as keyof typeof TYPOGRAPHY_SLOT_TO_FIELD
    ];
  return value[field];
}
  ...
  const resolvedValue =
    variable.kind === 'typography'
      ? resolveTypographyValue(
          requestedSlot,
          modeValue.value as TypographyTokenValue,
        )
...
function getAllowedSlotsForNode(
  node: RendererNode,
): readonly NodeSemanticSlot[] {
...
function resolveCanvasSemanticStateWithContext(
  ...
  return Object.fromEntries(
    CANVAS_SEMANTIC_SLOTS.map((slot) => [
      slot,
      resolveCanvasSemanticSlotWithContext(context, slot),
    ]),
...
function resolveNodeSemanticSlotWithContext(
  ...
  const styleSlotValue = getStyleSlotValue(
    family,
    styleId,
    context.document,
    slot,
  ...
  const variableResult = resolveVariableValue(
    context,
    styleSlotValue.variable_id,
    slot,
  );

  if (
    variableResult.source_kind === 'variable' &&
    variableResult.value !== undefined
  ) {
    return {
      slot,
...
export function resolveCanvasSemanticSlot(
  ...
  return resolveCanvasSemanticSlotWithContext(
    createResolutionContext(document, options),
    slot,
  );
}
...
export function resolveCanvasSemanticState(
  ...
  return resolveCanvasSemanticStateWithContext(
    createResolutionContext(document, options),
  );
}
  ...
  const nextNodes = Object.fromEntries(
    Object.entries(document.nodes).map(([nodeId, node]) => {
      ...
      for (const result of Object.values(resolvedSlots)) {
        ...
        renderStyle[NODE_SEMANTIC_SLOT_TO_RENDER_KEY[result.slot]] =
          result.value;
      }
