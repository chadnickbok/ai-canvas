...
function asStringNumberBooleanRecord(
  value: unknown,
...
function asNodeKind(value: unknown): RendererNodeKind | null {
  return NODE_KINDS.includes(value as RendererNodeKind)
    ? (value as RendererNodeKind)
    : null;
}

function normalizeCanvasAuthoring(rawValue: unknown): RendererCanvasAuthoring {
  const localValuesSource =
    isObject(rawValue) && isObject(rawValue.local_values)
      ? rawValue.local_values
      : {};
  const variableBindingsSource =
    isObject(rawValue) && isObject(rawValue.variable_bindings)
      ? rawValue.variable_bindings
      : {};
...
function normalizeNodeAuthoringForKind(
  ...
  const styleBindingsSource =
    isObject(rawValue) && isObject(rawValue.style_bindings)
      ? rawValue.style_bindings
      : {};

  const localValues = Object.fromEntries(
    getAllowedNodeSemanticSlots(nodeKind).map((slot) => [
      slot,
      typeof localValuesSource[slot] === 'string' ||
      typeof localValuesSource[slot] === 'number'
...
function normalizeTextNodePayload(
  rawValue: Record<string, unknown>,
): TextNode['text'] {
  const text = isObject(rawValue.text) ? rawValue.text : {};
  const legacyContent =
    typeof rawValue.text_content === 'string'
      ? rawValue.text_content
      : undefined;

  return {
    content:
      typeof text.content === 'string' ? text.content : (legacyContent ?? ''),
  };
}

function normalizeSvgDefinitions(
  value: unknown,
): SvgNodePayload['definitions'] {
  if (!Array.isArray(value)) {
    return undefined;
  }
...
function normalizeSvgNodePayload(
  rawValue: Record<string, unknown>,
): SvgNode['svg'] {
...
function normalizeSvgPrimitivePayload(
  rawValue: Record<string, unknown>,
): SvgVisualElementNode['svg_primitive'] {
  const primitive = isObject(rawValue.svg_primitive)
    ? rawValue.svg_primitive
    : {};
...
function normalizeFrameNode(
  ...
  return {
    ...
    authoring: normalizeNodeAuthoringForKind(
      'frame',
      rawValue.authoring,
    ) as FrameNode['authoring'],
  };
}

function normalizeRectangleNode(
  nodeId: string,
  rawValue: Record<string, unknown>,
): RectangleNode {
  return {
...
function normalizeTextNode(
  ...
  return {
    ...
    authoring: normalizeNodeAuthoringForKind(
      'text',
      rawValue.authoring,
    ) as TextNode['authoring'],
    text: normalizeTextNodePayload(rawValue),
  };
}

function normalizeSvgNode(
  nodeId: string,
  rawValue: Record<string, unknown>,
): SvgNode {
  return {
...
function normalizeOpaqueMetadata(
  value: unknown,
): Record<string, OpaqueValue> | undefined {
  if (!isObject(value)) {
    return undefined;
  }
...
function normalizeRendererSemanticSlots(
  value: unknown,
): RendererSemanticSlot[] {
  return dedupe(
    asStringArray(value).filter((slot) =>
      RENDERER_SEMANTIC_SLOTS.includes(slot as RendererSemanticSlot),
    ),
  ) as RendererSemanticSlot[];
}

function normalizeVariableModeValue(
  ...
  switch (kind) {
    ...
    case 'typography':
      ...
      return typeof rawValue.value.font_family === 'string' &&
        (typeof rawValue.value.font_size === 'string' ||
          typeof rawValue.value.font_size === 'number')
        ? {
            kind: 'value' as const,
            value: {
...
function normalizeVariable(
  ...
  const valuesByMode = Object.fromEntries(
    Object.entries(
      isObject(rawValue.values_by_mode) ? rawValue.values_by_mode : {},
    )
      .map(
        ([modeId, modeValue]) =>
          [modeId, normalizeVariableModeValue(kind, modeValue)] as const,
      )
      .filter(
        (
          entry,
          ...
          string,
          NonNullable<ReturnType<typeof normalizeVariableModeValue>>,
        ] => entry[1] !== null,
      ),
  );
...
function normalizeVariables(rawValue: unknown): RendererVariables {
  return {
    collections: normalizeRecord(
      rawValue && isObject(rawValue) ? rawValue.collections : {},
...
function normalizeStringStyleSlotValue(
  ...
  if (
    rawValue.kind === 'variable' &&
    typeof rawValue.variable_id === 'string'
  ) {
    return {
      kind: 'variable',
      variable_id: rawValue.variable_id,
...
function normalizePaintStyle(
  styleId: string,
  rawValue: unknown,
): RendererPaintStyle | null {
  if (!isObject(rawValue)) {
    return null;
  }
  ...
  return {
    ...
    slots: compactObject({
      'node.paint.background_color':
        normalizeStringStyleSlotValue(
          slotsSource['node.paint.background_color'],
        ) ?? undefined,
      'node.shape.border_radius':
        normalizeRenderStyleSlotValue(
          slotsSource['node.shape.border_radius'],
        ) ?? undefined,
      'node.paint.opacity':
        normalizeRenderStyleSlotValue(slotsSource['node.paint.opacity']) ??
        undefined,
    }),
  };
}
...
function normalizeTextStyle(
  ...
  return {
    ...
    slots: compactObject({
      'node.text.color':
        normalizeStringStyleSlotValue(slotsSource['node.text.color']) ??
        undefined,
      'node.typography.font_family':
        normalizeStringStyleSlotValue(
          slotsSource['node.typography.font_family'],
        ) ?? undefined,
      'node.typography.font_size':
        normalizeRenderStyleSlotValue(
          slotsSource['node.typography.font_size'],
        ) ?? undefined,
      'node.typography.font_weight':
        normalizeRenderStyleSlotValue(
          slotsSource['node.typography.font_weight'],
        ) ?? undefined,
      'node.typography.line_height':
        normalizeRenderStyleSlotValue(
          slotsSource['node.typography.line_height'],
        ) ?? undefined,
      'node.typography.letter_spacing':
        normalizeRenderStyleSlotValue(
          slotsSource['node.typography.letter_spacing'],
        ) ?? undefined,
    }),
  };
}
...
function normalizeStyles(rawValue: unknown): RendererStyles {
  ...
  return {
    paint: normalizeRecord(source.paint, (styleId, rawStyle) =>
      normalizePaintStyle(styleId, rawStyle),
    ),
    text: normalizeRecord(source.text, (styleId, rawStyle) =>
      normalizeTextStyle(styleId, rawStyle),
    ),
  };
}
...
function normalizeRootChildOrder(
  ...
  const missingTopLevelIds = topLevelNodeIds.filter(
...
function repairChildIds(nodes: Record<string, RendererNode>): void {
  ...
  for (const node of Object.values(nodes)) {
    ...
    node.child_ids = dedupe([
      ...validExistingChildren,
      ...backReferencedChildren,
    ]);
  }
}
...
function repairCanvasVariableBindings(
  ...
  for (const [slot, variableId] of Object.entries(
    ...
    if (variableId && !variablesById.has(variableId)) {
      delete authoring.variable_bindings[
        slot as keyof typeof authoring.variable_bindings
      ];
    }
  }
}
...
function repairNodeSemanticBindings(
  ...
  for (const node of Object.values(nodes)) {
    ...
    >;
    const styleBindings = node.authoring.style_bindings as Partial<
  ...
  for (const style of Object.values(styles.paint)) {
    ...
    for (const [slot, slotValue] of Object.entries(slots)) {
      if (
        ...
        !variablesById.has(slotValue.variable_id)
      ) {
        delete slots[slot];
      }
    }
...
function repairVariableAliasCycles(variables: RendererVariables): void {
  const { collectionsByVariableId, variablesById } =
    buildVariableIndex(variables);
...
export function normalizeDocument(
  ...
  const empty = createEmptyDocument({
    ...
    name,
    createdAt:
      isObject(rawDocument.source) &&
      typeof rawDocument.source.created_at === 'string'
        ? rawDocument.source.created_at
        : undefined,
  });

  const normalizedNodes = normalizeRecord(
    rawDocument.nodes,
    (nodeId, rawNode) => normalizeNode(nodeId, rawNode),
  );

  const normalizedScenes = normalizeRecord(
    rawDocument.scenes,
    (sceneId, rawScene) => normalizeScene(sceneId, rawScene),
  );
  ...
  for (const nodeId of rootChildIds) {
    propagateSceneIds(
      nodeId,
      normalizedScenes[nodeId] ? nodeId : null,
      normalizedNodes,
    );
  }

  const assets = normalizeRecord(rawDocument.assets, (assetId, rawAsset) =>
    normalizeAsset(assetId, rawAsset),
  );
  ...
  const normalizedDocument: RendererDocument = parseDocument({
    ...
    source: {
      ...
      ...compactObject({
        ...
        imported_at:
          isObject(rawDocument.source) &&
          typeof rawDocument.source.imported_at === 'string'
            ? rawDocument.source.imported_at
            : undefined,
        source_document_id:
          isObject(rawDocument.source) &&
          typeof rawDocument.source.source_document_id === 'string'
            ? rawDocument.source.source_document_id
            : undefined,
        source_file_name:
          isObject(rawDocument.source) &&
          typeof rawDocument.source.source_file_name === 'string'
            ? rawDocument.source.source_file_name
            : undefined,
        source_page_name:
          isObject(rawDocument.source) &&
          typeof rawDocument.source.source_page_name === 'string'
