...
describe('semantic resolution and materialization', () => {
  it('resolves local, variable, and typography slots with exact-slot provenance', () => {
    ...
    expect(
      resolveNodeSemanticSlot(document, 'text_1', 'node.text.color'),
    ).toEqual({
  ...
  it('repairs broken semantic references, trims alias cycles, and materializes semantic-owned keys', () => {
    ...
    expect(
      resolveCanvasSemanticSlot(document, 'canvas.background_color'),
    ).toEqual({
    ...
    expect(
      document.variables.collections.tokens.variables.a.values_by_mode,
    ).toEqual({});
    expect(
      document.variables.collections.tokens.variables.b.values_by_mode,
    ).toEqual({
      light: {
        kind: 'alias',
        variable_id: 'a',
