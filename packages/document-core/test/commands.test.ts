...
async function runCommands(
  ...
  return applyCommands(
    normalizedDocument,
    {
      ...
      commands,
      ...(options.baseRevision === undefined ? { base_revision: currentRevision } : { base_revision: options.baseRevision })
    },
    {
      currentRevision,
      measurementSurfaceAvailable: options.measurementSurfaceAvailable ?? true,
      refreshComputedLayout: options.refreshComputedLayout
        ? ({ document: refreshedDocument }) => options.refreshComputedLayout!(refreshedDocument)
...
describe("applyCommands", () => {
  ...
  it("reparents and reorders nodes while preserving structural invariants", async () => {
    ...
    expect(moved.document.root.child_ids).toEqual(["scene_home", "loose_1", "text_1"]);
  ...
  it("deletes variables with command-owned detach behavior for bindings, styles, and aliases", async () => {
    ...
    expect(result.document.variables.collections.tokens.variables.base_color).toBeUndefined();
    expect(result.document.variables.collections.tokens.variables.accent_color.values_by_mode.light).toEqual({
      kind: "value",
      value: "#112233"
    });
    expect(result.document.styles.text.text_body.slots["node.text.color"]).toEqual({
      kind: "value",
      value: "#112233"
    });
    expect(
      (result.document.nodes.text_1.authoring.local_values as Record<string, unknown>)[
        "node.text.color"
      ]
    ).toBe("#112233");
  ...
  it("supports variable-collection, variable, and style CRUD through the shared command path", async () => {
    ...
    expect(created.document.variables.collections.tokens.default_mode_id).toBe("dark");
    expect(created.document.variables.collections.tokens.description).toBe("Theme tokens");
    expect(created.document.variables.collections.tokens.variables.brand_color.name).toBe("Brand");
    expect(created.document.styles.text.text_body.slots["node.typography.font_size"]).toEqual({
      kind: "value",
      value: 20
    });
    ...
    expect(deletedCollection.document.variables.collections.tokens).toBeUndefined();
    expect(deletedCollection.document.styles.text.text_body.slots["node.text.color"]).toEqual({
      kind: "value",
      value: "#445566"
    });
  ...
  it("supports asset and SVG commands and clears asset references on delete", async () => {
    ...
    expect(created.document.nodes.rect_1.render_style.backgroundImage).toBe("url(asset://asset_1)");
    ...
    expect(deleted.document.nodes.rect_1.render_style.backgroundImage).toBeUndefined();
  });
