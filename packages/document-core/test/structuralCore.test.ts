...
describe("document-core structural model", () => {
  it("repairs structure, preserves typed payloads, and keeps computed layout optional", () => {
    const document = normalizeDocument(
      {
        ...
        root: {
          id: CANVAS_ROOT_ID,
          child_ids: ["scene_home", "missing_top", "rect_leaf", "scene_home", "legacy_text"]
        },
        scenes: {
          scene_home: {
            name: "Home",
    ...
    expect(getChildren(document, "scene_home").map((node) => node.id)).toEqual(["legacy_text"]);
    expect(collectSubtreeIds(document, "scene_home")).toEqual(["scene_home", "legacy_text"]);
