...
const sourceIconDocumentPath = path.join(appDir, "src/assets/branding/strapping_icon.icon");
const sourceSvgPath = path.join(appDir, "src/assets/branding/strapping_logo.svg");
...
function run(command, args, options = {}) {
  try {
    ...
    if (error instanceof Error && "code" in error && error.code === "ENOENT") {
      throw new Error(`Required tool "${command}" is not installed or not on PATH.`);
    }
    ...
    throw new Error(details ? `${command} failed: ${details}` : `${command} failed.`);
  }
}
...
function validateOutputs() {
  ...
  const [imageType, channels, topLeftPixel, bottomRightPixel] = identify.split("|");
...
function main() {
  if (process.platform !== "darwin") {
    throw new Error("Desktop icon generation requires macOS with Xcode Icon Composer.");
  }
  ...
  console.log(`Generated app icon assets in ${path.relative(appDir, iconOutputDir)}`);
}
