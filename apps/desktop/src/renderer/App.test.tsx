...
function createDesktopApiMock(overrides: Partial<DesktopApi> = {}) {
  ...
  const api: DesktopApi = {
    applyCommands:
      overrides.applyCommands ?? vi.fn(async () => ok({} as never)),
    createProject:
    ...
    getHistoryState:
      overrides.getHistoryState ?? vi.fn(async () => ok(historyState)),
...
afterEach(() => {
  ...
  Reflect.deleteProperty(
    window as Window & { aiCanvasApi?: DesktopApi },
    'aiCanvasApi',
  );
});
...
describe('App', () => {
  ...
  it('opens a project from the library and navigates into the workspace', async () => {
    ...
    const openProject = vi
      .fn<DesktopApi['openProject']>()
      .mockResolvedValue(ok(activeProject));
    const desktopApi = createDesktopApiMock({
  ...
  it('applies document_changed events to the visible workspace without a reload', async () => {
    ...
    const updatedDocument = createDocumentWithScene(
    ...
    try {
      ...
      expect(
        harness.container.querySelector('[data-node-id="scene_home"]'),
      ).not.toBeNull();
    } finally {
      harness.cleanup();
    }
