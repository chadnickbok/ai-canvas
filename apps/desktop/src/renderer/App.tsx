...
export function App() {
      ...
      setState((current) => ({
        ...
        errorMessage:
          error instanceof Error
            ? error.message
            : 'Failed to load the project library',
        isBusy: false,
      }));
    }
  };
    ...
    const unsubscribe = effectApi.subscribeToRuntimeEvents((runtimeEvent) => {
      ...
      if (!parsed.success) {
        setState((current) => ({
          ...current,
          errorMessage:
            'Received an invalid runtime event from the desktop main process.',
        }));
        return;
      }
  ...
  const handleCreateProject = async (input: CreateProjectInput) => {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to load the created project session';
  ...
  const measurementHost = api ? (
    <CommitLayoutMeasurementHost api={api} />
  ) : null;
