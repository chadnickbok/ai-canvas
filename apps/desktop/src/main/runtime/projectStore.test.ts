...
describe('ProjectStore asset persistence', () => {
  it('persists asset_store metadata outside current_document_json and hydrates it on reload', async () => {
    const { dbPath, store, tempDir } = await createTempStore(
      'ai-canvas-store-assets-',
    );
    ...
    expect(reopenedStore.resolveAssetFilePath(created.project.id, 'hero')).toBe(
      storedAssetPath,
    );
  ...
  it('migrates embedded assets into the disk-backed asset store and rewrites history snapshots', async () => {
    ...
    expect(history.undo[0]?.document.assets.hero.source.kind).toBe(
      'asset_store',
    );
    expect(
      reopenedStore.resolveAssetFilePath(created.project.id, 'hero'),
    ).not.toBeNull();
