...
async function createMainWindow(
  ...
  browserWindow.on('closed', () => {
    ...
    layoutMeasurementBridge.rejectAll(
...
async function bootstrap() {
  ...
  runtime.setComputedLayoutRefresher(async (input) => {
    ...
    const rootIds = resolveComputedLayoutRootIds(
      input.document,
      input.changed_node_ids,
    );
    ...
    const measuredLayouts = await layoutMeasurementBridge.measureDocumentLayout(
      browserWindow,
      {
        document: input.document,
        rootIds,
  ...
  const unsubscribeFromMcpStatus = mcpBridge.subscribeToStatusChanges(
    (status) => {
